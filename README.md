...
│       └── services/              # Reusable Home Manager service modules
├── profiles/
│   └── home/
│       └── common.nix             # Shared Home Manager profile
└── templates/
...
- macOS system and Homebrew management use nix-darwin. `modules/darwin/darwin.nix` is a nix-darwin module (not a Home Manager module) loaded at the `darwinConfigurations` level.
...
- After bootstrap, open a new terminal so the new `~/.zshrc` and `~/.zprofile`
  are loaded.
- Homebrew stays on `PATH` via `~/.zprofile` (set from `modules/home/programs/zsh.nix`),
...
Neovim reads the completion endpoint from `~/.config/nvim-fim.lua`, generated by the Spark Home Manager config.

## Serving other things through Caddy

Caddy is managed by `modules/home/services/caddy.nix` via `dotfiles.caddy.configText`. Modules can append additional site blocks with `lib.mkAfter`:
