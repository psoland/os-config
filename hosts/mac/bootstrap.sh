...
Usage: ./bootstrap.sh <personal|work>
  ...
  personal   psoland-mac        (user: psoland,      home: /Users/psoland)
  work       pettersoland-mac   (user: pettersoland,  home: /Users/pettersoland)
USAGE
  exit 1
fi
...
case "$PROFILE" in
  ...
  work)
    ...
    HOME_MANAGER_FLAKE="pettersoland-mac"
    ;;
esac
...
info "Starting macOS bootstrap for $USERNAME ($ARCH) — flake $HOME_MANAGER_FLAKE"
...
if [ "$PROFILE" = "work" ]; then
  ...
  cd "$DOTFILES_DIR"
  nix build ".#darwinConfigurations.${HOME_MANAGER_FLAKE}.system"
